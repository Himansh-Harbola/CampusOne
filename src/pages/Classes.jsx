  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{
        ...
        borderRadius: 16, padding: '22px', cursor: onClick ? 'pointer' : 'default',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <Badge variant="success">{cls.lectures.length} lectures</Badge>
          <Badge variant="muted">{cls.students.length} students</Badge>
        </div>
...
function TeacherClassDetail({ cls, onBack }) {
  ...
  function addLecture() {
    ...
    const lec = { id: 'l' + Date.now(), title: lecture.title, duration: lecture.duration || '—', date: new Date().toISOString().slice(0, 10), videoUrl: lecture.videoUrl || '' };
    setClasses(prev => prev.map(c => c.id === liveCls.id ? { ...c, lectures: [...c.lectures, lec] } : c));
  ...
  function toggleStudent(studentId) {
    setClasses(prev => prev.map(c => {
      ...
      return { ...c, students: inClass ? c.students.filter(s => s !== studentId) : [...c.students, studentId] };
    }));
  }

  return (
    <Page title={liveCls.name} subtitle={`${liveCls.code} · ${liveCls.students.length} students enrolled`}
      actions={<><Btn variant="secondary" onClick={onBack}>← Back</Btn><Btn onClick={() => setShowUpload(true)}>+ Upload Lecture</Btn></>}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20 }}>
        <div>
          <h3 style={{ margin: '0 0 14px', fontSize: 15, fontWeight: 600, color: 'var(--text-primary)', fontFamily: 'var(--font-serif)' }}>Video Lectures ({liveCls.lectures.length})</h3>
        ...
        <Card>
          <h3 style={{ margin: '0 0 14px', fontSize: 15, fontWeight: 600, color: 'var(--text-primary)', fontFamily: 'var(--font-serif)' }}>Students ({students.length})</h3>
          {students.map((s, i) => {
            ...
            return (
              <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: '1px solid var(--border-dim)' }}>
                ...
                <button onClick={() => toggleStudent(s.id)} style={{
                  padding: '4px 10px', borderRadius: 7,
                  border: `1px solid ${enrolled ? 'var(--success)' : 'var(--border-subtle)'}`,
                  background: enrolled ? 'var(--success-bg)' : 'transparent',
                  color: enrolled ? 'var(--success)' : 'var(--text-muted)',
                  fontSize: 11, cursor: 'pointer', whiteSpace: 'nowrap',
                  fontFamily: 'var(--font-body)',
                }}>{enrolled ? '✓ Enrolled' : '+ Add'}</button>
              </div>
            );
          })}
        </Card>
      ...
      {showUpload && (
        <Modal title="Upload Lecture" onClose={() => setShowUpload(false)}>
          <Input label="Lecture Title *" value={lecture.title} onChange={e => setLecture({ ...lecture, title: e.target.value })} placeholder="e.g. Introduction to Arrays" />
          <Input label="Duration" value={lecture.duration} onChange={e => setLecture({ ...lecture, duration: e.target.value })} placeholder="e.g. 45 min" />
          <Input label="YouTube Embed URL" value={lecture.videoUrl} onChange={e => setLecture({ ...lecture, videoUrl: e.target.value })} placeholder="https://www.youtube.com/embed/VIDEO_ID" />
...
function TeacherClasses() {
  const { user, classes, setClasses } = useApp();
  const [selected, setSelected] = useState(null);
  const [showCreate, setShowCreate] = useState(false);
  const [form, setForm] = useState({ name: '', code: '' });
  const myClasses = classes.filter(c => c.teacherId === user.id);

  if (selected) return <TeacherClassDetail cls={selected} onBack={() => setSelected(null)} />;

  function createClass() {
    if (!form.name || !form.code) return;
    setClasses(prev => [...prev, { id: 'c' + Date.now(), name: form.name, code: form.code, teacherId: user.id, students: [], lectures: [] }]);
    setForm({ name: '', code: '' });
    setShowCreate(false);
  }
    ...
    <Page title="My Classes" subtitle="Create and manage your classes" actions={<Btn onClick={() => setShowCreate(true)}>+ Create Class</Btn>}>
      {myClasses.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px,1fr))', gap: 16 }}>
          {myClasses.map(c => <ClassGridCard key={c.id} cls={c} onClick={() => setSelected(c)} />)}
        </div>
      )}
      {showCreate && (
...
function StudentClassDetail({ cls, onBack }) {
  ...
  return (
    <Page title={liveCls.name} subtitle={`${liveCls.code}${teacher ? ' · By ' + teacher.name : ''}`} actions={<Btn variant="secondary" onClick={onBack}>← Back</Btn>}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 20 }}>
        <div>
          <h3 style={{ margin: '0 0 14px', fontSize: 15, fontWeight: 600, color: 'var(--text-primary)', fontFamily: 'var(--font-serif)' }}>Lectures ({liveCls.lectures.length})</h3>
          {liveCls.lectures.length === 0 ? (
            <Card style={{ textAlign: 'center', padding: '44px' }}>
        ...
        <Card>
          ...
          {teacher && (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                ...
                <div>
                  <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{teacher.name}</p>
                  <p style={{ margin: 0, fontSize: 12, color: 'var(--text-muted)' }}>{teacher.department}</p>
                </div>
              </div>
            </>
          )}
          ...
          <p style={{ fontSize: 28, fontFamily: 'var(--font-display)', color: 'var(--text-primary)', margin: '0 0 16px' }}>{liveCls.students.length}</p>
          <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600 }}>Total Lectures</p>
          <p style={{ fontSize: 28, fontFamily: 'var(--font-display)', color: 'var(--text-primary)', margin: 0 }}>{liveCls.lectures.length}</p>
        </Card>
      </div>
    </Page>
  );
...
function StudentClasses() {
  ...
  function joinClass(classId) {
    setClasses(prev => prev.map(c => c.id === classId ? { ...c, students: [...c.students, user.id] } : c));
  }
      ...
      {enrolled.length > 0 && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px,1fr))', gap: 14, marginBottom: 28 }}>
            {enrolled.map(c => <ClassGridCard key={c.id} cls={c} teacher={allUsers.find(u => u.id === c.teacherId)} enrolled onClick={() => setSelected(c)} />)}
          </div>
        </>
      )}
      {available.length > 0 && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px,1fr))', gap: 14 }}>
            {available.map(c => <ClassGridCard key={c.id} cls={c} teacher={allUsers.find(u => u.id === c.teacherId)} onJoin={joinClass} />)}
          </div>
        </>
      )}
      {enrolled.length === 0 && available.length === 0 && (
        <Card style={{ textAlign: 'center', padding: '60px' }}>
          <p style={{ color: 'var(--text-muted)' }}>No classes available</p>
        </Card>
      )}
    </Page>
  );
