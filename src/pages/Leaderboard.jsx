                ...
                {[1, 0, 2].map(pos => {
                  ...
                  return (
                    <div key={s.id} style={{ flex: 1, textAlign: 'center', maxWidth: 160 }}>
                      ...
                      <div style={{
                        height: PODIUM_H[pos],
                        background: pos === 0 ? 'var(--accent-light)' : 'var(--bg-elevated)',
                        border: `1px solid ${pos === 0 ? 'var(--accent)' : 'var(--border-dim)'}`,
            ...
            {students.map((s, i) => {
              ...
              return (
                <div key={s.id} style={{
                  ...
                  borderBottom: i < students.length - 1 ? '1px solid var(--border-dim)' : 'none',
                  background: isMe ? 'var(--accent-light)' : 'transparent',
                }}>
                  <span style={{ width: 28, fontSize: i < 3 ? 18 : 13, fontWeight: 600, color: i < 3 ? RANK_COLORS[i] : 'var(--text-muted)', textAlign: 'center', flexShrink: 0 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 14, fontWeight: isMe ? 700 : 500, color: isMe ? 'var(--accent-text)' : 'var(--text-primary)' }}>
                      {s.name}{isMe && <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--accent)', background: 'rgba(201,125,46,0.15)', padding: '2px 8px', borderRadius: 10 }}>You</span>}
                    </p>
                    <p style={{ margin: '3px 0 0', fontSize: 12, color: 'var(--text-muted)' }}>{s.lecturesWatched || 0} lectures watched</p>
                  </div>
                  <div style={{ width: 70 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {user.role === 'student' && (
            <Card style={{ background: 'var(--accent-light)', border: '1px solid var(--accent)' }}>
              ...
              <p style={{ margin: '0 0 4px', fontSize: 40, fontFamily: 'var(--font-display)', color: 'var(--accent-text)' }}>#{myRank}</p>
